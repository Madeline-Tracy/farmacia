...
<table width="100%"  align="center">
          <tr>
            ...
            <td width="30%"><br><br>
              <div class="tituloFactura">Detalle de Ventas</div>
            </td>
            <td width="30%"><br><br>
              <table border="1">
                <tr>
...
<table width="100%">
  <tr style="text-align:center; font-size:15px;">
    <td width="100%"><b>PUNTO DE VENTA : </b>'.$aml['nombre'].'</td>
  </tr>
</table>
<table width="100%">
  <tr>
    <td width="100%"><b>VENTAS </b></td>
  </tr>
</table>
';
  $totalVenta=0;
  ...
  foreach($invventa->mostrarTodo("idsucursal=$idalmacen and cierrecaja=0 and estado=1") as $f)
 {
  ...
  <tr style="background-color:#f2f2f2; border:none;">
    <td class="cabecera4 tituloCenda tituloCenda" colspan="5" width="100%"><b> Usuario(a) : </b>'.$vu2['nombre'].'<b> Fecha : </b>'.$f['fechacreacion'].'<b> Hora : </b>'.$f['horacreacion'].'</td>
  </tr>
  <tr style="background-color:#f2f2f2; border:none;">
  ...
  $totalP=0;  
  foreach($invventadet->mostrarTodo("idventa=".$f['idinvventa']." and estado=1") as $vd)
  {
    $subtotal=$vd['precio']*$vd['cantidad'];
    $totalP=$totalP+$subtotal;
    $html=$html.'
    <tr>
      <td class="scenter">'.$vd['iditem'].'</td>
      <td class="">'.$vd['detalle'].'</td>
      <td class="scenter">'.$vd['cantidad'].'</td>
      <td class="sMoneda">'.number_format($vd['precio'], 2, '.', '').'</td>
  ...
  $html=$html.'
    <tr>
      ...
      <tr style="background-color:#f2f2f2; border:none;">
        <td colspan="3"></td>
        <td class="cabecera4 sMoneda">Total Venta Bs.</td>
        <td class="cabecera4 sMoneda"><b>'.number_format($totalP, 2, '.', '').'</b></td>
      </tr>
      <tr style="background-color:#f2f2f2; border:none;">
        <td colspan="3"></td>
        <td class="cabecera4 sMoneda">Efectivo.</td>
        <td class="cabecera4 sMoneda"><b>'.number_format($f['efectivo'], 2, '.', '').'</b></td>
      </tr>
      <tr style="background-color:#f2f2f2; border:none;">
        <td colspan="3"></td>
        <td class="cabecera4 sMoneda">Tarjeta.</td>
        <td class="cabecera4 sMoneda"><b>'.number_format($f['tarjeta'], 2, '.', '').'</b></td>
      </tr>
  </table>';
...
<table  width="100%" class="tabla">
  ...
  $totalGasto=0; 
  foreach($invgasto->mostrarTodo("idsucursal=".$idalmacen." and cierrecaja=0 and estado=1") as $f)
  {
   $vu2 = $vusuario2->muestra($f['usuariocreacion']);
    $html=$html.'
    <tr>
